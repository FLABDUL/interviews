...
public class PortfolioReconciliationImproved {
    ...
    public static List<String> reconcile(String[] initial, String[] function, String[] end) {
        ...
        for(String item : function){
            ...
            int tradeQuantity = 0;
            if (trade.equals("SL") && positions.getOrDefault(account, 0) < quantity) {
                // Handle error: you can't sell more stocks than you have
                throw new IllegalArgumentException("Selling more than owned!");
            }else if(trade.equals("SL")){
                tradeQuantity = -quantity;
            }else{
                tradeQuantity = quantity;
            }
            int price = Integer.parseInt(accountFunctionQuantityPrice[3]);
            positions.put(account, positions.getOrDefault(account, 0) + tradeQuantity);//safer to default
            cashChange += tradeQuantity*-price;
        }
        positions.put("Cash", positions.getOrDefault("Cash", 0)+cashChange);
        ...
        for(String item : end){
            ...
            String account = accountQuantity[0];
            int quantity = Integer.parseInt(accountQuantity[1]);
            if(positions.getOrDefault(account, 0) != quantity){
                int quantityChange = quantity - positions.getOrDefault(account, 0);
                element.append(account).append(" ").append(quantityChange);
                System.out.println("element " + element.toString());
                result.add(element.toString());//use element.toString() instead!
            }
        }
        return result;
    }
